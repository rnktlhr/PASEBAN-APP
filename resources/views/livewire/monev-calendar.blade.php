<div>
    ...
    <div wire:key="stats-{{ $tahun }}-{{ $dinas_id }}-{{ $status }}-{{ md5($search) }}"
        ...
        <div
            ...
            <div class="mono" style="font-size: 24px; font-weight: 800; color: #2e7d32; margin-top: 6px;"
                x-data="countUp({{ $monevTepatWaktu }})" x-text="count">0</div>
        </div>
        <div
            style="background: #fff; border: 1px solid var(--line); border-radius: var(--radius); padding: 18px; box-shadow: var(--shadow-sm);">
            <div
            ...
            <div class="mono" style="font-size: 24px; font-weight: 800; color: var(--red); margin-top: 6px;"
                x-data="countUp({{ $monevTerlambat }})" x-text="count">0</div>
        </div>
        <div
            style="background: #fff; border: 1px solid var(--line); border-radius: var(--radius); padding: 18px; box-shadow: var(--shadow-sm);">
            <div
                ...
                Keberhasilan</div>
            <div class="mono" style="font-size: 24px; font-weight: 800; color: var(--navy); margin-top: 6px;"
                x-data="countUp({{ $pctKeberhasilan }})"><span x-text="count">0</span>%</div>
        </div>
    </div>
    ...
    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap; margin-bottom: 20px;">
        ...
        <div style="display: flex; align-items: center; gap: 0;">
            ...
            <span class="mono"
                style="display: inline-flex; align-items: center; justify-content: center; width: 65px; height: 38px; border: 1px solid var(--line); background: #fff; font-size: 14px; font-weight: 700; color: var(--navy);">{{ $tahun }}</span>
    ...
    <div style="position: relative; background: #fff; border: 1px solid var(--line); border-radius: var(--radius); box-shadow: var(--shadow-sm);"
                    ...
                    @forelse($monevItems as $idx => $monev)
                        ...
                        <tr wire:key="desk-{{ $monev->kegiatan_id }}" style="border-bottom: 1px solid var(--line);"
                            ...
                            <td
                                ...
                                {{ $monev->kegiatanStatistik->nama ?? '-' }}</td>
                            <td style="padding: 14px 16px; color: var(--ink); vertical-align: middle;">
                                {{ $monev->kegiatanStatistik->dinas->singkatan ?? '-' }}</td>
                            <td style="padding: 14px 16px; text-align: center; vertical-align: middle;">
                                <span
                                    style="display: inline-block; width: 115px; text-align: center; padding: 4px 0; border-radius: 999px; font-size: 11px; font-weight: 600; color: {{ $statusEnum?->cssColor() ?? 'var(--muted)' }}; background: {{ $statusEnum?->cssBgColor() ?? '#f5f5f5' }};">
                            ...
                            @for($m = 1; $m <= 12; $m++)
                                ...
                                <td style="padding: 6px; text-align: center; vertical-align: middle;">
                                    <div style="width: 24px; height: 24px; border-radius: 4px; background: {{ $cellBg }}; opacity: {{ $opacity }}; transform: scale({{ $scale }}); margin: auto; transition: all 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) {{ $delayMs }}ms;"
                                        title="{{ $isRealisasi ? 'Realisasi' : ($isRencana ? 'Rencana' : '-') }}"></div>
                                </td>
                            @endfor
                        </tr>
                    @empty
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 12px;">
                        <div>
                            <div
                                ...
                                <span>#{{ $idx + 1 }}</span> &middot;
                                <span>{{ $monev->kegiatanStatistik->dinas->singkatan ?? '-' }}</span>
                            </div>
                            <div style="font-size: 14.5px; font-weight: 700; color: var(--navy); line-height: 1.35;">
                                {{ $monev->kegiatanStatistik->nama ?? '-' }}</div>
                        </div>
                        <div
                            style="flex-shrink: 0; display: flex; flex-direction: column; gap: 4px; align-items: flex-end;">
                            <span
                                style="display: inline-block; padding: 4px 8px; border-radius: 6px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: {{ $statusEnum?->cssColor() ?? 'var(--muted)' }}; background: {{ $statusEnum?->cssBgColor() ?? '#f5f5f5' }};">
                                {{ $statusEnum?->label() ?? ucfirst(str_replace('_', ' ', $monev->status)) }}
                            </span>
                        </div>
                    </div>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(12, 1fr); gap: 2px;">
                            @foreach(config('paseban.bulan') as $m => $namaBulan)
                                ...
                                <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;"
                                    title="{{ $namaBulan }} - {{ $isRealisasi ? 'Realisasi' : ($isRencana ? 'Rencana' : '-') }}">
                                    <div class="mono"
                                        style="font-size: 9px; color: var(--muted); font-weight: 600; letter-spacing: -0.5px; transition: color 0.3s ease {{ $delayMs }}ms;">
                                        {{ substr($namaBulan, 0, 1) }}</div>
                                    <div
                                        style="width: 100%; height: 24px; border-radius: 4px; background: {{ $cellBg }}; opacity: {{ $opacity }}; border: {{ $border }}; transition: all 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) {{ $delayMs }}ms;">
                                    </div>
                                </div>
                            @endforeach
                        </div>
        ...
        <div
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <button type="button" @click="if(page < Math.ceil({{ count($monevItems) }} / perPage)) page++"
                    style="padding: 6px 12px; border: 1px solid var(--line); background: #fff; border-radius: 6px; font-size: 13px; cursor: pointer;"
                    :style="page >= Math.ceil({{ count($monevItems) }} / perPage) ? 'opacity: 0.5; cursor: not-allowed;' : ''">Selanjutnya</button>
            </div>
        </div>
    </div>
