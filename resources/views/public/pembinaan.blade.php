        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: 24px;">
            ...
            <div style="background: #fff; border: 1px solid var(--line); border-radius: 12px; overflow: hidden; box-shadow: var(--shadow-sm);" x-data="{ perPage: 10, page: 1 }">
                ...
                <div class="mobile-only">
                    @forelse($rekapKehadiran as $idx => $rekap)
                    <div style="border-bottom: 1px solid var(--line); padding: 16px 20px; display: flex; flex-direction: column; gap: 12px;" x-show="page === Math.ceil({{ $idx + 1 }} / perPage)">
                        <div style="font-size: 14.5px; font-weight: 700; color: var(--navy); line-height: 1.35;">{{ $rekap['dinas']->nama }}</div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr; gap: 8px; margin-top: 4px; background: var(--navy-50); padding: 12px; border-radius: 8px;">
                            @foreach($sesiPembinaan as $index => $sesi)
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-size: 11px; font-weight: 600; color: var(--navy);">Sesi {{ $index + 1 }}</span>
                                @if(isset($rekap['kehadiran'][$sesi->id]) && $rekap['kehadiran'][$sesi->id])
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed rgba(0,0,0,0.1); padding-top: 8px; margin-top: 4px;">
                                ...
                                <span style="font-size: 13px; font-weight: 800; color: var(--orange);">{{ $rekap['total'] }}/{{ $totalSesi }}</span>
                            </div>
                        </div>
                    </div>
                    @empty
            ...
            <div class="mobile-only">
                ...
                <div style="border-bottom: 1px solid var(--line); padding: 16px 20px; display: flex; flex-direction: column; gap: 12px;" x-show="page === Math.ceil({{ $idx + 1 }} / perPage)">
                    <div style="display: flex; gap: 16px; align-items: flex-start;">
                        ...
                        <div style="flex: 1;">
                            <div style="font-weight: 700; color: var(--navy); margin-bottom: 4px; font-size: 14.5px; line-height: 1.35;">{{ $materi->judul }}</div>
                            <div style="display: flex; gap: 8px; align-items: center;">
                                ...
                                @endif
                                <span style="font-size: 11px; color: var(--muted);">{{ $materi->tanggal ? $materi->tanggal->format('d M Y') : '-' }} &middot; {{ $materi->ukuran_file ?? 'Link' }}</span>
                            </div>
                        </div>
                    </div>
                    <div style="margin-top: 4px;">
                        @if($materi->jenis == 'VIDEO' && $materi->link_url)
                        <a href="{{ $materi->link_url }}" target="_blank" style="display: flex; justify-content: center; align-items: center; gap: 6px; padding: 10px 16px; border: 1px solid var(--line); color: var(--ink); border-radius: 8px; font-weight: 600; font-size: 13px; text-decoration: none; background: #fff; box-shadow: var(--shadow-sm);">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>
                            Putar Video
                        </a>
                        @elseif($materi->file_path || $materi->link_url)
