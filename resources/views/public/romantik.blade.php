    ...
    <div class="flex-col-mobile" style="margin-bottom: 24px;">
        ...
        <div class="w-full-mobile" style="display: flex; gap: 12px; align-items: center;">
            ...
            <div class="w-full-mobile" style="position: relative;">
                ...
                <input type="text" x-model="search" class="w-full-mobile" placeholder="Cari kegiatan atau OPD..." style="padding: 10px 14px 10px 36px; border: 1px solid var(--line); border-radius: 8px; font-size: 13.5px; width: 280px; outline: none; color: var(--ink); background: #fff; box-shadow: var(--shadow-sm); transition: border-color 0.2s, box-shadow 0.2s;" onfocus="this.style.borderColor='var(--orange)'; this.style.boxShadow='0 0 0 3px rgba(235, 137, 27, 0.1)';" onblur="this.style.borderColor='var(--line)'; this.style.boxShadow='var(--shadow-sm)';">
            </div>
        </div>
    </div>
        ...
        <div class="mobile-only">
            @forelse($romantik as $item)
                @php
                    ...
                    $stBps = $item->status_bps instanceof \App\Enums\StatusBps ? $item->status_bps : \App\Enums\StatusBps::tryFrom($item->status_bps);
                @endphp
                <div style="border-bottom: 1px solid var(--line); padding: 16px 20px; display: flex; flex-direction: column; gap: 12px;" x-show="(dinasFilter === '' || '{{ $item->kegiatanStatistik->dinas_id }}' === dinasFilter) && (search === '' || $el.dataset.search.toLowerCase().includes(search.toLowerCase()))" data-search="{{ addslashes(strtolower($item->kegiatanStatistik->dinas->singkatan ?? '')) }} {{ addslashes(strtolower($item->kegiatanStatistik->nama)) }}">
                    <div>
            ...
            @empty
                <div style="padding: 32px; text-align: center; color: var(--muted);">Belum ada pengajuan Romantik untuk tahun ini.</div>
            @endforelse
        </div>
    </div>
</div>
