...
<div class="container" style="padding: 40px 32px 0;">
    <div style="margin-bottom: 24px;">
        <h2 style="margin: 0; font-size: 22px; font-weight: 800; color: var(--navy);">Capaian Metadata Tahun {{ $tahun }}</h2>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-bottom: 32px;" class="metadata-cards-grid">
        ...
        <div class="scroll-reveal" style="background: #fff; border: 1px solid var(--line); border-radius: var(--radius); padding: 24px; box-shadow: var(--shadow-sm); display: flex; flex-direction: column; --delay: {{ $index * 100 }}ms;">
            ...
            <div style="flex: 1; display: flex; justify-content: center; align-items: center; width: 100%; min-height: 200px;">
                <div id="chart-{{ $card['id'] }}" style="width: 100%; display: flex; justify-content: center; cursor: pointer;" onclick="document.querySelector('[name=jenis]').value = '{{ $card['id'] }}'; document.getElementById('filter-form').submit();"></div>
            </div>

            <div style="display: flex; justify-content: center; gap: 18px; font-size: 12px; color: var(--muted); margin-top: 8px;">
                <div style="display: flex; align-items: center; gap: 6px; cursor: pointer;" onclick="document.querySelector('[name=jenis]').value = '{{ $card['id'] }}'; document.getElementById('filter-form').submit();">
